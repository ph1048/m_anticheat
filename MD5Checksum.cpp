...
NOTES:			Provides an interface to the CMD5Checksum class. 'strFilePath' name should 
				...
				exception.
*****************************************************************************************/
LPTSTR CMD5Checksum::GetMD5(LPCSTR strFilePath)
{
	//open the file as a binary file in readonly mode, denying write access 
	HANDLE File= CreateFileA(strFilePath,GENERIC_READ,FILE_SHARE_READ,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);

	//the file has been successfully opened, so now get and return its checksum
	return GetMD5(File);
}

LPTSTR CMD5Checksum::GetMD5(LPCWSTR strFilePath)
{
	//open the file as a binary file in readonly mode, denying write access 
	HANDLE File= CreateFileW(strFilePath,GENERIC_READ,FILE_SHARE_READ,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
...
*****************************************************************************************/
LPTSTR CMD5Checksum::GetMD5(HANDLE File)
{
		...
		while (( ReadFile(File, Buffer, nBufferSize,&bytesRead,NULL)) > 0 && bytesRead)
		{
...
*****************************************************************************************/
LPTSTR CMD5Checksum::GetMD5(BYTE* pBuf, UINT nLength)
{
	//entry invariants
...
LPTSTR CMD5Checksum::Final()
{
	...
	LPTSTR strMD5 = new CHAR[33];
	LPTSTR tmp = new CHAR[2];
	lstrcpy(strMD5,"");
